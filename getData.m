function [X1, Y1, U1, V1, Z1, X2, Y2, U2, V2, Z2, X3, Y3] = getData(S, frame)
    ...
    [X1, Y1, U1, V1, Z1] = getAirData(S, frame, X3, Y3);
    [X2, Y2, U2, V2, Z2] = getLiquidData(S, frame, X3, Y3);
end 
...
function [X1, Y1, U1, V1, Z1] = getAirData(S, frame, X3, Y3)
    ...
    film_height_at_air_x = interp1(X3, Y3, x_air, 'linear', 'extrap'); % 1 x 50 

    liquid_above_interface = Y1 < film_height_at_air_x; % 99 x 50 logical matrix

    U1(liquid_above_interface) = NaN;
    V1(liquid_above_interface) = NaN;
end

function [X2, Y2, U2, V2, Z2] = getLiquidData(S, frame, X3, Y3)
